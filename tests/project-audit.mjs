...
add('asset-restore:data-present', fs.existsSync(path.join(root,'data/asset-restore-data.js')) && fs.existsSync(path.join(root,'src/core/asset-restore-system.js')) && fs.readFileSync(path.join(root,'index.html'),'utf8').includes('src/core/asset-restore-system.js'), 'asset restore system');
add('asset-restore:audit-present', fs.existsSync(path.join(root,'test-results/asset-restore-audit.json')) && JSON.parse(fs.readFileSync(path.join(root,'test-results/asset-restore-audit.json'),'utf8')).failed === 0, 'asset restore audit');
add('asset-restore:ui-present', fs.readFileSync(path.join(root,'script.js'),'utf8').includes('assetRestoreMiniHTML') && fs.readFileSync(path.join(root,'script.js'),'utf8').includes('prepareGuidedAssetRestore') && fs.readFileSync(path.join(root,'style.css'),'utf8').includes('asset-restore-card'), 'system card + restore plan');
add('asset-restore:guide-present', fs.existsSync(path.join(root,'docs/ASSET_RESTORE_GUIDE_F21.md')), 'docs/ASSET_RESTORE_GUIDE_F21.md');
add('visual-hotfix:data-present', fs.existsSync(path.join(root,'data/visual-hotfix-data.js')) && fs.existsSync(path.join(root,'src/core/visual-hotfix-system.js')) && fs.readFileSync(path.join(root,'index.html'),'utf8').includes('src/core/visual-hotfix-system.js'), 'visual hotfix system');
add('visual-hotfix:audit-present', fs.existsSync(path.join(root,'test-results/visual-hotfix-audit.json')) && JSON.parse(fs.readFileSync(path.join(root,'test-results/visual-hotfix-audit.json'),'utf8')).failed === 0, 'visual hotfix audit');
add('visual-hotfix:ui-present', fs.readFileSync(path.join(root,'script.js'),'utf8').includes('visualHotfixMiniHTML') && fs.readFileSync(path.join(root,'script.js'),'utf8').includes('runVisualHotfixAudit') && fs.readFileSync(path.join(root,'style.css'),'utf8').includes('visual-hotfix-card'), 'system card + scroll');

add('scroll:creator-fix-present', fs.readFileSync(path.join(root,'style.css'),'utf8').includes('F22') && fs.readFileSync(path.join(root,'style.css'),'utf8').includes('asset-path-hint'), 'scroll + paths visible');
